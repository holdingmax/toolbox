...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setLoading(true)
    try {
      await resetPasswordApi(token as string, nueva)
      setOk(true)
    } catch {
      setError('Token inválido o expirado. Pedí un nuevo link desde la pantalla de inicio de sesión.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="min-h-screen flex">
      ...
      {/* ── Panel izquierdo — branding (idéntico a LoginPage) ─────────────── */}
      <div className="hidden lg:flex lg:w-[52%] xl:w-1/2 relative overflow-hidden">
        ...
        />
        <svg className="absolute bottom-0 left-0 w-[58%] pointer-events-none" viewBox="0 0 520 180" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="relative z-10 flex flex-col w-full">
          <div className="flex-1 flex flex-col items-center justify-center px-12 py-16 text-center">
            <div className="flex items-center gap-5 mb-7">
              ...
              <span className="text-[2.8rem] font-bold text-white tracking-[0.18em] leading-none">TOOLBOX</span>
            </div>
        ...
        <div className="flex-1 flex items-center justify-center px-8">
          <div className="w-full max-w-[390px]">
            ...
            >
              {!token ? (
                <>
                  <h2 className="text-text-primary text-2xl font-bold mb-1">Link inválido</h2>
                  <p className="text-text-secondary text-sm mb-6">
                    Este link no incluye la información necesaria para restablecer tu contraseña. Pedí uno nuevo desde la pantalla de inicio de sesión.
                  </p>
                  <Link
                    to="/login"
                    className="block text-center w-full bg-gradient-to-r from-[#7c3aed] to-[#5b21b6] hover:from-[#8b5cf6] hover:to-[#6d28d9] text-white font-semibold py-3.5 rounded-lg transition-all text-sm"
                  >
                    Volver al login
                  </Link>
                </>
                ...
                <>
                  <h2 className="text-text-primary text-2xl font-bold mb-1">¡Listo!</h2>
                  <p className="text-text-secondary text-sm mb-6">
                ...
                <>
                  <div className="mb-7">
                    <h2 className="text-text-primary text-2xl font-bold mb-1">Nueva contraseña</h2>
                    <p className="text-text-secondary text-sm">Ingresá y confirmá tu nueva contraseña</p>
                  </div>
